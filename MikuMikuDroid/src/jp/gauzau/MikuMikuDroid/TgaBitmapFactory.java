...
//				bmp = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);				
	...
	public static Bitmap decodeFile(String file, int scale) throws IOException {
		RandomAccessFile raf = new RandomAccessFile(file, "r");
		MappedByteBuffer m = raf.getChannel().map(FileChannel.MapMode.READ_ONLY, 0, raf.length());
		m.position(0);
		m.order(ByteOrder.LITTLE_ENDIAN);
		...
		m.position(12);
		short w = (short) (m.getShort() / scale);
		short h = (short) (m.getShort() / scale);
		byte  depth = m.get();
		byte  mode  = m.get();
//		Log.d("TgaBitmapFactory", String.format("%s = %d x %d in depth %d", file, w, h, depth));
		...
		if(depth == 24 || depth == 32) {
			...
			if(type == 2) { // no compression
				for(int i = 0; i < h; i ++) {
					for(int j = 0; j < w; j ++) {
						...
						// skip (scale - 1) pixel
						m.position(m.position() + (depth / 8) * (scale - 1));
					}
					// skip (scale - 1) line
					m.position(m.position() + (depth / 8) * w * scale);
				}				
			} else {	// RLE compression
				int pos = 0;
				while(pos < w * h * scale * scale) {
					int header = 0x000000ff & m.get();
					if(header < 128) {		// normal pixel
						for(int i = 0; i < header + 1; i++) {
							int color = getPixel(m, depth);
							setPixel(bmp, pos++, w, h, mode, color, scale);
						}
					} else {				// RLE pixel
						header -= 127;
						int color = getPixel(m, depth);
			...
			/*
			OutputStream os = new FileOutputStream(file.replace(".tga", ".png"));
			bmp.compress(Bitmap.CompressFormat.PNG, 100, os);
			os.close();
			*/
		}
