...
public class Miku {
	...
	private PMDParser mParser;
	private VMDParser mVMDParser;
	
	private ArrayList<Material> mRendarList;
	private HashMap<String, TexBitmap> mBitmap;
	...
	private void reconstructFace() {
		...
		// find base face
		if(mParser.getFace() != null) {
			for(Face s: mParser.getFace()) {
				if(s.face_type == 0) {
					mFaceBase = s;
					break;
				}
	...
	private void makeIndexSortedBuffers() {
		mIndexMaps = new int[mParser.numVertex()];
        ...
        ByteBuffer abb = ByteBuffer.allocateDirect(mParser.numVertex()*8*4);
        abb.order(ByteOrder.nativeOrder());
        mAllBuffer = abb.asFloatBuffer();
        
        // weight buffer
        ByteBuffer wbb = ByteBuffer.allocateDirect(mParser.numVertex()*2*4);
        wbb.order(ByteOrder.nativeOrder());
        mWeightBuffer = wbb.asFloatBuffer();
        
		// index buffer
        ByteBuffer ibb = ByteBuffer.allocateDirect(mParser.getIndex().size()*2);
    	...
    	for(Integer idx: mParser.getIndex()) {
    		if(mIndexMaps[idx] < 0) {	// not mapped yet
            	Vertex ver = mParser.getVertex().get(idx);
	...
	private void reconstructMaterial(int max_bone) {
    	...
    	for(Material mat: mParser.getMaterial()) {
	...
	private void reconstructMaterial1(Material mat, int offset, int max_bone) {
		...
		ArrayList<Vertex> ver = mParser.getVertex();
	...
	private void buildBoneRenameIndex(Material mat, int max_bone) {
        ByteBuffer rbb = ByteBuffer.allocateDirect(mParser.numVertex()*3); 
        rbb.order(ByteOrder.nativeOrder());
        mat.rename_index = rbb;

        for(int i = 0; i < mIndexMaps.length; i++) {
        	Vertex ver = mParser.getVertex().get(i);
        	if(mIndexMaps[i] >= 0) {
	...
	private void buildBoneNoMotionRenameIndex() {
        ...
        for(int i = 0; i < mParser.numVertex(); i++) {
        	rbb.put((byte) 0);
        	rbb.put((byte) 0);
        	rbb.put((byte) 100);
        }
        ...
        for(Material m: mParser.getMaterial()) {
        	m.rename_index = rbb;
        }
	}
	
	private int renameBone1(HashMap<Integer, Integer> rename, int veridx, ArrayList<Vertex> ver, int acc) {
		int idx = ver.get(mParser.getIndex().get(veridx)).bone_num_0;
		Integer i = rename.get(idx);
		if(i == null) {
			rename.put(idx, acc++);
		}
		idx = ver.get(mParser.getIndex().get(veridx)).bone_num_1;
		i = rename.get(idx);
		if(i == null) {
			rename.put(idx, acc++);
		}
...
//				gl11.glBindBuffer(GL11.GL_ARRAY_BUFFER, 0);
        	...
        	if(mat.texture != null) {
                ...
                gl.glTexParameterx(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER, GL10.GL_LINEAR);
                gl.glTexParameterx(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER, GL10.GL_LINEAR);
        		gl.glBindTexture(GL10.GL_TEXTURE_2D, mBitmap.get(mat.texture).tex);
        	} else {
        		gl.glActiveTexture(GL10.GL_TEXTURE1);
		...
		*/
		...
		for(int r = 0; r < max; r++) {
        	...
        	if(mat.texture != null) {
        		...
        		GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mBitmap.get(mat.texture).tex);
        	...
        	float w = 0.6f;
        	float wi = 0.6f;
        	GLES20.glUniform4f(color, mat.face_color[0] * wi, mat.face_color[1] * wi, mat.face_color[2] * wi, mat.face_color[3]);
...
//        	GLES20.glBlendColor(0, 0, 0, mat.face_color[3]);
	...
	private void setFace(Face f, float i) {
		FacePair mp = mVMDParser.findFace(f, i, mFacePair);
		FaceIndex m = mVMDParser.interpolateLinear(mp, i, mFaceIndex);
...
//		preCalcIK();
	...
	public void setBonePosByVMDFrame(float i) {
		ArrayList<Bone> ba = mParser.getBone();
		int max = ba.size();
...
//		ccdIK();
	...
	public void preCalcIK() {
		...
		for(int frame = 0; frame < mVMDParser.maxFrame(); frame += 5) {
		...
		for(Entry<Short, ArrayList<MotionIndex>> entry: mhash.entrySet()) {
			Bone b = mParser.getBone().get(entry.getKey());
	...
	public void preCalcKeyFrameIK() {
		...
		for(IK ik: mParser.getIK()) {
			...
			while(target != -1) {
				Bone b = mParser.getBone().get(target);
			...
			while(effecter != -1) {
				...
				if(b != null) {
					...
					b = mParser.getBone().get(effecter);
	...
	private void setBoneMatrix(Bone b, float idx) {
		MotionPair mp = mVMDParser.findMotion(b, idx, mMpWork);
		Motion m = mVMDParser.interpolateLinear(mp, idx, mMwork);
		if(m != null) {
			...
			if(b.parent == -1) {
				...
				Bone p = mParser.getBone().get(b.parent);
	...
	private void ccdIK1(IK ik) {
		Bone effecter = mParser.getBone().get(ik.ik_bone_index);
		Bone target   = mParser.getBone().get(ik.ik_target_bone_index);

		getCurrentPosition(effecterVecs, effecter);

		for(int i = 0; i < ik.iterations; i++) {
			for(int j = 0; j < ik.ik_chain_length; j++) {
				Bone b = mParser.getBone().get(ik.ik_child_bone_index[j]);
				...
				if(b.is_leg) {
					if(i == 0) {
						Bone base = mParser.getBone().get(ik.ik_child_bone_index[ik.ik_chain_length-1]);
						getCurrentPosition(targetInvs, b);
						getCurrentPosition(effecterInvs, base);
						
						double eff_len = Matrix.length(
								effecterVecs[0] - effecterInvs[0],
								effecterVecs[1] - effecterInvs[1],
								effecterVecs[2] - effecterInvs[2]
						);
						double b_len = Matrix.length(
								targetInvs[0] - effecterInvs[0],
								targetInvs[1] - effecterInvs[1],
								targetInvs[2] - effecterInvs[2]
						);
						double t_len = Matrix.length(
								targetVecs[0] - targetInvs[0],
								targetVecs[1] - targetInvs[1],
								targetVecs[2] - targetInvs[2]
						);
	...
	private void clearUpdateFlags(Bone root, Bone b) {
		while(root != b) {
			b.updated = false;
			if(b.parent != -1) {
				b = mParser.getBone().get(b.parent);
			} else {
				return ;
			}
		}
    ...
    public boolean invertM(float[] mInv, int mInvOffset, float[] m, int mOffset) {
        ...
        float det =
                mInvSrcs[0] * mInvDsts[0] + mInvSrcs[1] * mInvDsts[1] + mInvSrcs[2] * mInvDsts[2] + mInvSrcs[3]
                        * mInvDsts[3];
	...
	public void calcToonTexCoord(float x, float y, float z) {
        ByteBuffer tbb = ByteBuffer.allocateDirect(mParser.numVertex()*2*4); 
...
//        	Log.d("Miku", "V: " + String.valueOf(p));
	...
	public void readAndBindTexture(GL10 gl) {
		...
		mBitmap = new HashMap <String, TexBitmap> ();
		for(int i = 0; i < mParser.numMaterial(); i++) {
			Material mat = mParser.getMaterial().get(i);
			if(mat.texture != null) {
				if(mBitmap.get(mat.texture) == null) {
					// read
					TexBitmap tb = new TexBitmap();
					
					tb.bmp = loadPicture(mat.texture, 2);
			    	Log.d("Miku", mat.texture + ": " + String.valueOf(tb.bmp.getWidth()) + "x" + String.valueOf(tb.bmp.getHeight()) + " at row size " + 
			    			String.valueOf(tb.bmp.getRowBytes()) + "byte in " + tb.bmp.getConfig().name());
					mBitmap.put(mat.texture, tb);
...
//		            GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, GLES20.GL_NEAREST_MIPMAP_NEAREST);
					...
					if(tb.bmp.hasAlpha()) {	// workaround
						...
						buf.position(0);
						GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, tb.bmp.getWidth(), tb.bmp.getHeight(), 0, 
...
//					GLES20.glGenerateMipmap(GLES20.GL_TEXTURE_2D);
	...
	public void readToonTexture(){
		...
		for(int i = 0; i < 11; i++) {
			TexBitmap tb = new TexBitmap();
    		tb.bmp = loadPicture(mParser.getToonFileName(i), 1);
	    	Log.d("Miku", mParser.getToonFileName(i) + ": " + String.valueOf(tb.bmp.getWidth()) + "x" + String.valueOf(tb.bmp.getHeight()) + " at row size " + 
	    			String.valueOf(tb.bmp.getRowBytes()) + "byte in " + tb.bmp.getConfig().name());
			mToon.add(tb);
		}
	}
